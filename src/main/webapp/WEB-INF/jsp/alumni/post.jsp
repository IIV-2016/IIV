...
<body class="header-fixed header-fixed-space">
	<div class="wrapper">
		...
		<div class="breadcrumbs">
			<div class="container">
				<h1 class="pull-left">Alumni Post</h1>
		...
		<div class="bg-color-light">
			<div class="container content-sm">
						...
						<ul class="post-shares post-shares-lg">
							...
							<li>
								...
								<c:choose>
							  		<c:when test="${likeHistory == 0}">
										<a href="<%=request.getContextPath()%>/alumni/likes/${post.id}/${currentUserId}">
											<i class="rounded-x icon-heart"></i>
											<span>${post.likes}</span>
										</a>
									</c:when>
									<c:otherwise>
										<a href="<%=request.getContextPath()%>/alumni/likesremove/${post.id}/${currentUserId}">
							...
							<sec:authorize access="isAuthenticated()">
								...
								<c:if test="${currentUserId eq post.userId}">
									<li>
										<a href="<%=request.getContextPath()%>/alumni/delete/${post.id}">
											<i class="rounded-x icon-close"></i>
										</a>
									</li>
									<li>
										<a href="<%=request.getContextPath()%>/alumni/update/${post.id}">
				...
				<form action="<%=request.getContextPath()%>/comment/write/submit" name="comment" method="post" id="sky-form3" class="sky-form comment-style">
					...
					<input type="hidden" name="table" id="table" value="alumni_comment" class="form-control">
					<fieldset>
	...
	<script type="text/javascript">
		...
		function commentList(){
			var postId = ${post.id};
			var allData = { "postId": postId, "table": "alumni_comment"};
		    $.ajax({
