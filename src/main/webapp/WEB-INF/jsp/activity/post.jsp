...
<body class="header-fixed header-fixed-space">
							...
							<li>
								...
								<c:choose>
							  		<c:when test="${likeHistory == 0}">
										<a href="<%=request.getContextPath()%>/activity/likes/${post.id}/${currentUserId}">
											<i class="rounded-x icon-heart"></i>
											<span>${post.likes}</span>
										</a>
									</c:when>
									<c:otherwise>
										<a href="<%=request.getContextPath()%>/activity/likesremove/${post.id}/${currentUserId}">
	...
	<script type="text/javascript">
		...
		function commentList(){
			var postId = ${post.id};
			var allData = { "postId": postId, "table": "activity_comment"};
		    $.ajax({
